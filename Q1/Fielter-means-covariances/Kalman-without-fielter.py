...
LastEditTime: 2025-12-08 23:17:40
...
def run_without_filter(ssm, y_obs):
    ...
    x_pred[0]= y_obs[0][0]
    x_pred[0][1] = y_obs[0][1]
    P_pred[0][0][0] = ssm.R[0][0]
    P_pred[0][1][1] = ssm.R[1][1]
    for k in range(T):
...
if __name__ == "__main__":
    ...
    plt.plot(x_without_filter[:, 0], label='Withoout fielter($x_t$)', color='green', )
    ...
    plt.plot(x_without_filter[:, 0], x_without_filter[:, 1], label='Trajectory without fielter', color='green', )
