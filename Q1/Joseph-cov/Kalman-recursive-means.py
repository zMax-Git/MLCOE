...
def run_kalman_filter_joseph(ssm, y_obs):
    """
...
if __name__ == "__main__":
    ...
    x_est_joseph, P_est_joseph, x_before_correction_joseph, P_before_correction_joseph, kalman_gain_joseph = run_kalman_filter_joseph(lgssm, y_obs)
    
    # plot_matplotlib()
    plt.plot(P_est_joseph[:, 0, 0], label='Joseph Stabilized Kalman Filter')
    plt.plot(P_est[:, 0, 0], label='Standard Kalman Filter')
    plt.legend()
    plt.savefig('Joseph-cov.png')
    plt.show()
